...
def test_openai_provider_populates_section_analysis(monkeypatch, client) -> None:
    old_ai_provider = settings.ai_provider
    old_openai_api_key = settings.openai_api_key
    settings.ai_provider = "openai"
    settings.openai_api_key = "test-key"

    def fake_openai_analysis(section: PaperSection) -> tuple[str, str]:
        return (
    ...
    monkeypatch.setattr(
        "app.services.section_analyzer.analyze_section_with_openai",
    ...
    try:
        paper = Paper(
            user_id=1,
            title="OpenAI Test",
            original_filename="openai-test.pdf",
            stored_filename="openai-test.pdf",
            status="completed",
        )
        db.add(paper)
        db.commit()
